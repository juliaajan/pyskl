...
model = dict(
    ...
    backbone=dict(
        ...
        pretrained='./work_dirs/julia/AblationStudies/hands_shoulders_arms_flip_128px_lr_0_01/epoch_20.pth',
        in_channels= N_SHOULDERS_ARMS_LANDMARKS + 2 * N_HAND_LANDMARKS, #number of keypoints
        base_channels=32,
        num_stages=3,
        out_indices=(2, ),
        stage_blocks=(4, 6, 3),
...
ann_file = 'julia/WLASL300/pyskl_mediapipe_annos_2d_denormalized_NOFACE_SHOULDERS_ARMS.pkl' #TODO 

#Define kepoints for "Flip" Module
#point 0 is the nose which is centered, and therefore neither left nor right keypoint
left_shoulder_arm = [4, 5, 6, 8, 10, 12, 14, 16, 18, 20, 22]
right_shoulder_arm = [1, 2, 3, 7, 9, 11, 13, 15, 17, 19, 21]
#left and right hand keypoints each with 21 kps starting at index 23, left hand is extracted first
left_hand = list(range(23, 44))
right_hand = list(range(44, 65)) 
left_kp = left_shoulder_arm + left_hand
right_kp = right_shoulder_arm + right_hand
...
log_level = 'INFO'
work_dir = './work_dirs/julia/AblationStudies/hands_shoulders_arms_flip_128px_dataaugment_lr_0_001' #TODO
