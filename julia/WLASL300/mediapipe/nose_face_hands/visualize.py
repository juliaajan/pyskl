...
skeleton_map = dict(
    ...
    mediapipe_face=[
        (0, 1, 'f'), (1, 2, 'f'), (2, 3,'f'), #right eyebrow
        (4, 5, 'f'), (5, 6, 'f'), (6, 7, 'f'), #left eyebrow
        (8, 9, 'f'), (9, 10, 'f'), (10, 11, 'f'), (11, 8, 'f'), #right eye
        (12, 13, 'f'), (13, 14, 'f'), (14, 15, 'f'), (15, 12, 'f'), #left eye
        (16, 17, 'f'), (17, 18, 'f'), (18, 19, 'f'), (19, 16, 'f') #mouth
    ]
)
...
def display_keypoints_on_video(anno_pickle_path, path_video, output_path):
    ...
    show_nose=True,       # Körper anzeigen
    thre=0.0,             # Confidence threshold
    fps=25
)
    
    output_file = os.path.join(output_path, f'skeleton_vis_nose_face_hands_{video_id}.mp4')
    video.write_videofile(output_file)
    print(f"Saved visualized video to {output_file}")



def Vis2DPoseMediaPipe(item, thre=0.0, out_shape=(1080, 1920), fps=25, video=None, show_face=True, show_hands=True, show_nose=True):
    tx = cp.deepcopy(item)
    item = tx
    ...
    face_edges = skeleton_map['mediapipe_face']
    ...
    for i in tqdm(range(total_frames)):
        ...
        for m in range(kp_frame.shape[0]):  # For each person
            ske = kp_frame[m]
            # Draw nose (nose keypoints with connections to eyes and mouth)
            if show_nose:
                #kp 0 is the nose
                x_nose, y_nose = int(ske[0][0]), int(ske[0][1])
                nose_conf = ske[0][2] if kp.shape[-1] == 3 else 0.0
                if nose_conf > thre:
                    frames[i] = cv2.circle(frames[i], (x_nose, y_nose), 5, (255, 0, 0), -1)
            
            # Draw face keypoints
            if show_face:
                if show_nose:
                    face_kps = ske[1:21] #20 face keypoints, starting at index 1
                else:
                    face_kps = ske[0:20] #20 face ekypoints starting at index 0
                for st, ed, co in face_edges:
                    j1, j2 = face_kps[st], face_kps[ed]
            ...
            # Draw hands
            if show_hands:
                if show_face and show_nose:
                    # Left hand: keypoints 21-41
                    left_hand = ske[21:42]
                    # Right hand: keypoints 42-62
                    right_hand = ske[42:63]
                elif show_face:
                    # Left hand: keypoints 20-40
                    left_hand = ske[20:41]
                    # Right hand: keypoints 41-61
                    right_hand = ske[41:62]
                else:
                    # Left hand: keypoints 0-20
                    left_hand = ske[0:21]
                    # Right hand: keypoints 21-42
                    right_hand = ske[21:42]
