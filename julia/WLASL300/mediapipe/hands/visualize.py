...
def display_keypoints_on_video(anno_pickle_path, path_video, output_path):
    ...
    output_file = os.path.join(output_path, f'skeleton_vis_hands_{video_id}.mp4')
    video.write_videofile(output_file)
    print(f"Saved visualized video to {output_file}")



def Vis2DPoseMediaPipe(item, thre=0.0, out_shape=(1080, 1920), fps=25, video=None, show_hands=True):
    tx = cp.deepcopy(item)
    item = tx
    ...
    for i in tqdm(range(total_frames)):
        ...
        for m in range(kp_frame.shape[0]):  # For each person
            ske = kp_frame[m]
            assert len(ske) == 42, f"Expected 42 keypoints, got {len(ske)}"
